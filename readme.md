...
Из лекции 4 (Slide 10–12), грамматика с индексами:
...
> В нашей грамматике правило присваивания после факторизации (лекция 5, Slide 22):
> ```
> A  → ID B
...
Из лекции 4 (Slide 11–12), таблица генератора ОПС:
...
Из лекции 5 (Slide 2), после преобразования к ННФГ и факторизации:
...
Из лекции 4 (Slide 13) и лекции 5 (Slides 7, 12):
...
Из лекции 4 (Slide 13): *«Каждый элемент ОПС состоит из двух частей: 1) тип элемента; 2) если элемент — операнд, то ссылка на соответствующую таблицу»*.

Из лекции 4 (Slide 14): *«каждый элемент в магазине состоит из двух частей: вида содержимого и собственно содержимого»*.
...
Сгенерированная ОПС (запись в лекционном стиле):
```
x  a  b  2  *  +  :=
```
...
Из лекции 4 (Slide 18), входная цепочка:
```
M[k] := a * L[k, j+d]
```
...
Из лекции 5 (Slide 8), входная цепочка:
```
...
Из лекции 5 (Slide 10), входная цепочка:
```
while a > b do a := b
```
...
Из лекции 5 (Slide 13), входная цепочка:
```
